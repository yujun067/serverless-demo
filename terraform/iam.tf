  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          aws_dynamodb_table.user_scores.arn,
          "${aws_dynamodb_table.user_scores.arn}/index/*"
        ]
      },
      {
        Effect = "Allow"
...
# Fargate Task Role Policy for DynamoDB
resource "aws_iam_role_policy" "fargate_task_dynamodb" {
  name = "${local.name_prefix}-fargate-task-dynamodb-policy"
...
# Fargate Execution Role Policy Attachment
